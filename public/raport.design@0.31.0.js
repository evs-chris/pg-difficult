...
${f.opts.map(o => `${o.name} - ${o.type}\n  ${o.desc}\n`).join('\n')}` : ''}`;
    ...
    registerOperatorDoc({
        op: 'highlight',
        sig: [{ proto: 'str => str', desc: 'Parses the given string as an expression and returns an HTML string with AST-node classes suitable for displaying as colored code.' }],
    });
    let clipEl;
