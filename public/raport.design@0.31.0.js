...
<li><code>width</code>, <code>w</code> - sets the width of the block in <code>rem</code>, e.g., <code>|w=10|this is 10rem wide</code>. The width does not include any padding or margin, in contrast to the behavior of the width and margin properties of raport widgets. Width may also be set as a percentage of its container, making it easier to align text within a widget e.g., <code>|w=100%,align=center|this is centered in the container</code>.</li>
    ...
    let builtinDoc = true;
    function registerOperatorDoc(doc) {
...
${f.opts.map(o => `${o.name} - ${o.type}\n  ${o.desc}\n`).join('\n')}` : ''}`;
    ...
    `;
        ...
        async buildRoot(skipSources, sample) {
            ...
            const report = this.get('report');
            const res = new index.Root(cloneDeep(report.context), { parameters: this.get('params'), special: { definition: report } });
            res.log = this.log;
                    ...
                    while (loc && parts.length) {
                        ...
                        if (loc && loc.type === 'repeater') {
                            ...
                            if (parts[0] === 'row') {
                                ...
                                ctx = index.extend(ctx, { value: Object.assign({}, values) });
                            }
        ...
        sortManual(ctx) {
            ...
            if (c.type === 'container' && Array.isArray(c.layout)) {
                const all = c.layout.map((l, i) => ({ widget: c.widgets[i], layout: l }));
                const res = index.evaluate({ all }, `sort(all '[=>layout.1 ?? layout.y =>layout.0 ?? layout.x]')`);
                ctx.set({
    ...
    function gatherLabelIDs(widget, ids) {
        if (!widget)
            return ids;
        if ('id' in widget)
            ids[widget.id] = Object.keys(ids).length;
        if ('widgets' in widget)
            for (const w of widget.widgets)
                gatherLabelIDs(w, ids);
        return ids;
    }
    ...
    exports.docText = docs$1;
    exports.docs = docs;
    exports.highlight = highlight;
    exports.registerFormatDoc = registerFormatDoc;
    exports.registerOperatorDoc = registerOperatorDoc;
