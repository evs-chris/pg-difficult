...
The titular bat-tool on this utility belt. Diffs can be started on multiple databases, and the server will throttle the notifications, and tell the attached clients to ask for new changes as they stream in. Diffs are recorded in groups called segments. Changing the segment name from the control panel or any diff display will start a new segment, and all running diffs share the segment name.

Entries can be cleared from the control panel while diffs are still running to reset saved and active state.
...
There is a special fetch data source for reports that can bypass CORS requirements by using the server's HTTP client.

Scratch pads can also be used as data sources by entering the path to the desired scratch pad. This can be quite a bit more convenient than having embedded JSON sources in provided sources, especially if there are a few different iterations of a data source that can be run against.
...
The database tree will allow you to filter the databases in the tree, and the Query All tool will allow you to aggregate queries across all databases that are in the tree. This is also where multi-database reports get their list of databases to query. Query All queries that are used to provide a result will export that result in a way that makes it available to raport scratch pads.
...
The easiest way would be to download a release from the GitHub releases. The safest way is to clone this repo and build it yourself, so you can have complete control over what it gets to access. Running used to be a bit more complicated when the server was based on Deno.
...
The binaries are plopped out in the build directory. For developement purposes, you can run `go build .` in the repo root, followed by `./pg-difficult --client-dev`, which will use the public folder rather than the public.zip created by the build script. If you don't want to build the full list of the usual binaries, you must first create a zip archive of the public directory, typically with something like `zip -r public.zip public`. The client files are zipped up to be embedded in the server binary because it saves ~14MB, and that makes me happy. It also appears to have no real effect on serving the static assets.
...
Once the main application is mounted, a websocket connection is established with the server. The server is then asked for the current status and set of recorded entries. The client is also notified any time additional entries are made, and it will request the entries using the id of the last received entries as a delta point. If the websocket gets disconnected, the client will attempt to reconnect it, blocking the UI until it can communicate with the server again.
...
The client takes advantage of local storage in the browser to save connection details, queries, and settings. This has the pro of allowing different setups to be saved in the same browser by changing the port that the server is listening on. It also has the con that you can't take your settings between browsers, unless you set up synchronization with a CouchDB server (or something compatible).

The client is also happy to run in serverless mode, where it gives you the ability to reload saved diffs and schemas. You can also use the scratch and reporting systems where the functionality doesn't rely on the server - like query and HTTP sources.
